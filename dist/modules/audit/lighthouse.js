...
exports.runPerformanceAudit = void 0;
...
const runPerformanceAudit = async (url) => {
    ...
    try {
        ...
        page.on('response', response => {
            ...
            totalBytes += size;
            if (headers['content-encoding']?.match(/gzip|br|deflate/))
                isCompressed = true;
            if (headers['x-cache']?.includes('HIT') || headers['cf-cache-status'] === 'HIT')
                cacheHits++;
        });
        ...
        const mLcp = await mobilePage.evaluate(() => window.lcp || 0);
        await browser.close();
        return {
